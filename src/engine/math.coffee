class @Vector3
  constructor: (@x, @y, @z) -> @magnitude = Math.sqrt((Math.pow(@x, 2)) + (Math.pow(@y, 2)) + (Math.pow(@z, 2)))
  add: (v) -> new Vector3(@x + v.x, @y + v.y, @z + v.z)
  sub: (n) -> new Vector3(@x - n.x, @y - n.y, @z - n.z)
  mul: (c) -> new Vector3(@x * c, @y * c, @z * c)
  dot: (v) -> @x * v.x + @y * v.y + @z * v.z
  cross: (v) -> new Vector3((@y * v.z) - (@z * v.y), (@z * v.x) - (@x * v.z), (@x * v.y) - (@y * v.x))
  normalized: -> @mul(1 / Math.sqrt(@x * @x + @y * @y + @z * @z))
  asNormal: ->
    v = @normalized()
    new Normal3(v.x, v.y, v.z)
  reflectedOn: (n) -> @add(n.mul((@dot(n) * -2.0))).mul(-1)

class @Normal3
  constructor: (@x, @y, @z) ->
  mul: (c) -> new Normal3(@x * c, @y * c, @z * c)
  add: (n) -> new Normal3(@x + n.x, @y + n.y, @z + n.z)
  dot: (v) -> @x * v.x + @y * v.y + @z * v.z

class @Point3
  constructor: (@x, @y, @z) ->
  add: (v) -> new Point3(@x + v.x, @y + v.y, @z + v.z)
  subPoint: (p) -> new Vector3(@x - p.x, @y - p.y, @z - p.z)
  subVector: (v) -> new Point3(@x - v.x, @y - v.y, @z - v.z)
...
class @Mat3x3
  ...
  changeCol1: (v) -> new Mat3x3(v.x, @m12, @m13, v.y, @m22, @m23, v.z, @m32, @m33)
  changeCol2: (v) -> new Mat3x3(@m11, v.x, @m13, @m21, v.y, @m23, @m31, v.z, @m33)
  changeCol3: (v) -> new Mat3x3(@m11, @m12, v.x, @m21, @m22, v.y, @m31, @m32, v.z)
