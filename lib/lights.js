...
(function () {
    ...
    this.DirectionalLight = (function () {
        ...
        DirectionalLight.prototype.illuminates = function (point, world) {
            ...
            ray = new Ray(point.sub(this.direction.normalized().mul(50)), this.direction.normalized());
            hit = world.hit(ray);
    ...
    this.DirectionalLight = (function () {
        ...
        function DirectionalLight(color, castsShadows, position) {
            this.color = color;
            this.castsShadows = castsShadows;
        ...
        DirectionalLight.prototype.illuminates = function (point, world) {
            ...
            ray = new Ray(this.position, point.sub(this.position).normalized());
            hit = world.hit(ray);
            if(hit === null || Math.round(hit.t * 100000.0) / 100000.0 >= Math.round(ray.tOf(point) * 100000) / 100000.0) {
                return true;
            }
    ...
    this.Spotlight = (function () {
        ...
        Spotlight.prototype.illuminates = function (point, world) {
            ...
            ray = new Ray(this.position, point.sub(this.position).normalized());
            hit = world.hit(ray);
            alpha = Math.acos(this.direction.dot(point.sub(this.position)) / (this.direction.magnitude * point.sub(this.position).magnitude));
